...
public abstract class OrderListCheckboxAction implements OrderListAction {
    ...
    public void render(Graphics g, OrderListMaster master, int x, int y, int w, int h, int mouseState, boolean actionEnabled) {
        ...
        if(actionEnabled) {
            int borderThickness = master.getStyleNumbers().get("checkbox" + styleKeyVariation + ".border.thickness");
            g.setColor(master.getColors().get("checkbox" + styleKeyVariation + ".border.color"));
            g.fillRect(x - borderThickness, y + buttonVGap - borderThickness, buttonSize + 2*borderThickness, buttonSize + 2*borderThickness);

            g.setColor(master.getColors().get("checkbox" + styleKeyVariation + ".background"));
